    ...
    <div class="panel panel-default" style="border:none;">
      ...
      <div class="panel-body">
        <form method="post" style="text-align:left" class="form-inline">
          ...
          <label style="margin-left: 20px;">充值订单号:</label>
          <input type="text" placeholder="请输入充值订单号" class="form-control sel">
      ...
      <table class="layui-table">
        ...
        <tbody>
          <tr v-for="(item,index) in arr" :key="index">
            ...
            <td>{{item.payordernum}}</td>
            <td>{{item.orderaccount}}</td>
            <td>{{item.gamename}}</td>
            <td>{{item.area}}</td>
            <td>{{item.rolename}}</td>
            <td>{{item.channelmethod}}</td>
            <td>{{item.paytime}}</td>
          </tr>
        </tbody>
      </table>
    ...
    data() {
      return {
        ...
        arr: [{
            ...
            "payordernum": "100000",
            "orderaccount": "1100.00",
            "gamename": "倚剑刀狂-IOS",
            "area": "双线一服",
            "rolename": "后正奇",
            "channelmethod": "七分钱支付宝支付",
            "paytime": "2018-03-28 13:43:34"
          },
          {
            "playaccount": "1235605660",
            "channelaccount": "1235605660",
...
<style scoped>
  @import url('../scss/ordersearch.css');
